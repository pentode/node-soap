...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    var _xsiTypeSchema;
    var _xsiType = elementAttributes['xsi:type'];
    if (_xsiType) {
      var _type = splitNSName(_xsiType);
      var _typeDef = self.findParameterObject(xmlns[_type.namespace], _type.name);
      if (_typeDef) {
        _xsiTypeSchema = _typeDef.description(self.definitions);
      }
    }

    if (topSchema && topSchema[name + '[]'])
      name = name + '[]';
    stack.push({name: originalName, object: obj, schema: (_xsiTypeSchema || (topSchema && topSchema[name])), id: attrs.id, nil: hasNilAttribute});
  };
